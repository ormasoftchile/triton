...
import {
  ...
  buildUnionFind,
  layoutUnionFind,
} from '../src/diagrams/triton/ds/unionfind/unionfind.js';
...
describe('graph', () => {
  ...
  it('renders an arrow marker def only in directed mode', () => {
    const directed = layoutGraph(
      graph.parseMermaid('nodegraph\n  directed\n  A -> B\n'),
      defaultTheme,
    );
...
describe('nodegraph edge highlight', () => {
  it('parses edge with kind active', () => {
    const ir = graph.parseMermaid(
      ['nodegraph', '  directed', '  A -> B : active', '  B -> C'].join('\n'),
    );
  ...
  it('renders active edges with primary color and thicker stroke', () => {
    const ir = graph.parseMermaid(
      ['nodegraph', '  directed', '  node A : parse', '  node B : emit', '  A -> B : active'].join(
        '\n',
      ),
    );
