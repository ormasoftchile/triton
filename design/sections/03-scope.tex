...
\begin{description}
  ...
  \item[Non-Timeline Chart Views] No aggregate or statistical chart views of timeline data --- for example, per-year histograms, bar charts, or pie charts. The ``YEARLY CHART'' view shown in the Gitline reference (\S\ref{sec:target-outputs}) is explicitly excluded.
  
  \textbf{Rationale:} Timeline Compiler renders \emph{timelines}. Aggregate or statistical chart views are a different visualization grammar served well by general charting tools; including them would dilute the system's focus and its deterministic timeline-layout mandate.
