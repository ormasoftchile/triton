...
Layer 1: Domain IRs (grammar-specific)
  +-------------+  +-------------+  +-------------+
  | Timeline IR |  |   Flow IR   |  |  Graph IR   |
  +-------------+  +-------------+  +-------------+
        |                |                |
        v                v                v
  [Timeline        [Flow Layout    [Graph Layout
   Layout Engine]   Engine]         Engine]
        |                |                |
        +-------+--------+-------+--------+
                |
                v
Layer 2: Scene IR (universal)
  +------------------------------------------+
  |  Rect, Circle, Line, Path, Text, Group   |
  |  Effects, Animation Hints                |
  +------------------------------------------+
                |
                v
  [Rendering Backends: SVG, Skia, PPTX]
\end{Verbatim}
\caption{The two-IR-layer model: diverse Domain IRs compile to unified Scene IR}
\label{fig:two-ir-model}
\end{figure}
...
\begin{tabular}{llll}
\toprule
\textbf{Grammar} & \textbf{Domain Entities} & \textbf{Status} & \textbf{Priority} \\
\midrule
