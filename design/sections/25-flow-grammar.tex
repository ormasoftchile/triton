...
Fully implemented in \texttt{packages/core/src/grammars/flow/} with Sugiyama four-phase layered layout (cycle detection, longest-path rank assignment, barycenter-based crossing minimization, Brandes--K\H{o}pf coordinate assignment). FlowTheme with light and dark variants. \textbf{Deferred:} general non-layered layouts (force-directed, stress-majorization).
...
These correspond to the \emph{node-link/graph} family's \emph{topology} and \emph{pipeline/sequence} kinds (Section~\ref{sec:family-nodelink}). The animated-arrow variant (flowing dashes along connectors) is a first-class pattern in this grammar.
