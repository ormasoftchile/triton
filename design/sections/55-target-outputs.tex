...
\begin{itemize}
  ...
  \item \textbf{Alternate view (out of scope):} The ``YEARLY CHART'' tab shows a
        per-year histogram of the same data. This is a non-timeline chart view and is
        \emph{explicitly out of scope} (see \S\ref{sec:scope}); it is noted here only
        to delimit the boundary, not as a supported render target.
\end{itemize}
...
# YEARLY CHART tab: out of scope (non-timeline aggregate view); see Scope Boundaries
...
The ``YEARLY CHART'' tab is a non-timeline aggregate view and is \emph{out of scope}
(see \S\ref{sec:scope}); the timeline card view is the target reproduced here.
...
\begin{description}
  ...
  \item[Render-6: ``YEARLY CHART'' aggregate view --- out of scope.]
        The T5 reference includes a ``YEARLY CHART'' tab (a per-year histogram). This
        is a non-timeline aggregate visualization and is \emph{out of scope}
        (\S\ref{sec:scope}); it is not a deferred feature but an explicit boundary.
\end{description}
