...
\medskip\noindent
\textbf{Grammar sequencing note.} The grammars that have received full concrete specifications are:
\begin{enumerate}
  \item \textbf{Flow Grammar} (Grammar \#2, Section~\ref{sec:flow-grammar}) --- directed node-link diagrams; requires Sugiyama layered layout.
  \item \textbf{Sequence Grammar} (Grammar \#3, Section~\ref{sec:sequence-grammar}) --- participant-message interaction diagrams; \emph{de-risked} first new grammar because its layout is deterministic-by-construction (no graph auto-layout required).
  \item \textbf{Tree Grammar} (Grammar \#4, Section~\ref{sec:tree-grammar}) --- rooted hierarchy diagrams; de-risked via Buchheim--J{\"u}nger--Leipert $O(n)$ tidy-tree layout.
\end{enumerate}
