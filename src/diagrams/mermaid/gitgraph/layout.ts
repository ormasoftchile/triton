...
export function layoutGitgraph(ir: GitgraphDocument, theme: ResolvedTheme): LayoutResult {
  ...
  for (let lane = 0; lane < ir.lanes; lane++) {
    ...
    const startX = bp
      ? Math.min(bp.x, ...(xs.length ? xs : [bp.x]))
      : Math.min(0, ...(xs.length ? xs : [0]));
    const endX = xs.length ? Math.max(...xs) : startX;
    elements.push(
      p.path(
        `M ${rhu(cx(startX))} ${rhu(cy(lane))} L ${rhu(cx(endX))} ${rhu(cy(lane))}`,
        hue(lane),
        3,
        { opacity: 0.55 },
      ),
    ...
    if (name)
      elements.push(
        p.text(
          name,
          margin,
          rhu(cy(lane) + typography.smallFontSize * 0.35),
  ...
  for (const c of ir.commits) {
    ...
    if (c.type === 'highlight') {
      elements.push(
        p.rect(
          { x: rhu(x - 9), y: rhu(y - 9), width: 18, height: 18 },
          palette.warning,
          palette.background,
          2,
          { rx: 3 },
    ...
    elements.push(
      p.text(c.id, rhu(x), rhu(y + 22), typography.smallFontSize, palette.textMuted, {
        anchor: 'middle',
      }),
    );
    if (c.tag) {
      ...
      elements.push(
        p.rect(
          { x: rhu(x - w / 2), y: rhu(y - 34), width: rhu(w), height: 17 },
          palette.warning + '22',
          palette.warning,
          1,
          { rx: 3 },
      ...
      elements.push(
        p.text(c.tag, rhu(x), rhu(y - 22), typography.smallFontSize, palette.text, {
          anchor: 'middle',
          weight: 'bold',
        }),
      );
