...
interface RoutedSegment {
  ...
  y1: number; // top-left
  x2: number;
  y2: number; // bottom-right (x2 >= x1, y2 >= y1)
}
...
function labelInBox(lx: number, ly: number, boxes: NodeBox[], padding = 8): boolean {
  return boxes.some(
    (b) =>
      lx > b.x - padding &&
      lx < b.x + b.width + padding &&
      ly > b.y - padding &&
      ly < b.y + b.height + padding,
  );
}
...
export function layoutClass(ir: ClassDocument, theme: ResolvedTheme): LayoutResult {
  ...
  const sizeOf = (c: ClassBox): { w: number; h: number; attrH: number } => {
    const memberTexts = [
      ...
      ...c.attributes.map((m) => m.text),
      ...c.methods.map((m) => m.text),
    ];
  ...
  const nodes: GraphNode[] = ir.classes.map((c) => ({
    id: c.name,
    width: sizes.get(c.name)!.w,
    height: sizes.get(c.name)!.h,
  }));
  ...
  const realColumnXs = [...new Set(allRealBoxes.map((b) => b.x + b.width / 2))].sort(
    (a, b) => a - b,
  );
  ...
  function scoreLane(
    ...
    const labelPenalty =
      labelMid != null && labelInBox(labelMid.x, labelMid.y, allRealBoxes) ? 200 : 0;
  ...
  for (let ri_ = 0; ri_ < ir.relations.length; ri_++) {
    ...
    const span_ =
      Math.abs(a_.x + a_.width / 2 - (b_.x + b_.width / 2)) +
  ...
  edgeEntries.sort((ea, eb) => classOrder[ea.cls] - classOrder[eb.cls] || eb.span - ea.span);
  ...
  for (const { ri, cls } of edgeEntries) {
    ...
    const fromPt =
      fromPortMap2.get(`${a.id}:${fromWall}`)?.get(ri) ??
    ...
    if (bends && bends.length > 0) {
      ...
      const blockingF = allRealBoxes.filter((nb) => {
        return (
          (exitY > nb.y && exitY < nb.y + nb.height) ||
          (tgtMidY_ > nb.y && tgtMidY_ < nb.y + nb.height)
        );
      });
      const adaptiveLeftX_F =
        blockingF.length > 0
      ...
      const laneXsD = [adaptiveLeftX_BD, realMinX - CLEARANCE, ...sweepCandidates].filter(
        (x) => x < srcLeft_,
      );
      ...
      const laneXsE = [adaptiveRightX_CE, rightMarginX, ...sweepCandidates].filter(
        (x) => x > srcRight_,
      );
      ...
      const laneXsF = [adaptiveLeftX_F, realMinX - CLEARANCE, ...sweepCandidates].filter(
        (x) => x < tgtLeft_,
      );
      ...
      elements.push(
        ...endMarker(
          p,
          effectiveFromPt,
          wallDir(effectiveFromWall, effectiveFromPt),
          headFrom,
          palette,
        ),
      );
      elements.push(
        ...endMarker(p, effectiveToPt, wallDir(effectiveToWall, effectiveToPt), headTo, palette),
      );
    } else {
      ...
      if (cls === 'direct-same-column') {
        ...
        // ── X1 candidates ──
        const midYRaw = [(fy + ty) / 2, fy + LAYER_GAP / 2, ty - LAYER_GAP / 2];
        for (const midY of dedup(midYRaw, 2)) {
          const segsX1: Array<[number, number, number, number]> =
            Math.abs(fx - tx) < 1
              ? [[fx, fy, tx, ty]]
              : [
                  [fx, fy, fx, midY],
        ...
        for (const midX of dedup(midXRaw, 2)) {
          const segsX2: Array<[number, number, number, number]> =
            Math.abs(fy - ty) < 1
              ? [[fx, fy, tx, ty]]
              : [
                  [fx, fy, midX, fy],
      ...
      for (const c of directCandidates) {
        ...
        const boxesForScoring =
          c.strategy === 'X1' || c.strategy === 'X2' ? allRealBoxes : interBoxesDirect;
        const score = scoreLane(
          c.laneX,
          c.segments,
          boxesForScoring,
      ...
      elements.push(
        ...endMarker(
          ...
          effectiveFromPt,
          wallDir(effectiveFromWall, effectiveFromPt),
          isFlipped ? r.rightHead : r.leftHead,
          palette,
        ),
      );
      elements.push(
        ...endMarker(
          p,
          effectiveToPt,
          wallDir(effectiveToWall, effectiveToPt),
          isFlipped ? r.leftHead : r.rightHead,
          palette,
        ),
      );
    }
    ...
    if (r.label) {
      ...
      elements.push(
        p.text(r.label, rhuInt(labelX), rhuInt(my - 4), memFont, palette.textMuted, {
          anchor: labelAnchor,
        }),
      );
    }
    ...
    if (r.leftCard) {
      const o = cardOffset(
        isFlipped ? effectiveToWall : effectiveFromWall,
        isFlipped ? effectiveToPt : effectiveFromPt,
      );
      elements.push(p.text(r.leftCard, rhu(o.cx), rhu(o.cy), memFont, palette.textMuted));
    }
    if (r.rightCard) {
      const o = cardOffset(
        isFlipped ? effectiveFromWall : effectiveToWall,
        isFlipped ? effectiveFromPt : effectiveToPt,
      );
  ...
  for (const c of ir.classes) {
    ...
    elements.push(
      p.rect(
        { x: rhu(x), y: rhu(y), width: rhu(s.w), height: rhu(s.h) },
        palette.surface,
        palette.border,
        1.4,
        { rx: 4 },
    ...
    elements.push(
      p.text(c.name, rhuInt(x + s.w / 2), rhu(ty), nameFont, palette.text, {
        weight: 'bold',
        anchor: 'middle',
      }),
    );
    if (c.stereotype) {
      ty += memFont + 2;
      elements.push(
        p.text(`«${c.stereotype}»`, rhuInt(x + s.w / 2), rhu(ty), memFont, palette.textMuted, {
          anchor: 'middle',
        }),
      );
    }
    ...
    elements.push(
      p.path(
        `M ${rhu(x)} ${rhu(headerBottom)} L ${rhu(x + s.w)} ${rhu(headerBottom)}`,
        palette.border,
        1,
      ),
    );
    ...
    for (const m of c.attributes) {
      elements.push(p.text(m.text, rhu(x + 8), rhu(ay), memFont, palette.text));
      ay += lineH;
    }
    const attrBottom = y + s.attrH;
    elements.push(
      p.path(
        `M ${rhu(x)} ${rhu(attrBottom)} L ${rhu(x + s.w)} ${rhu(attrBottom)}`,
        palette.border,
        1,
      ),
    );
    ...
    for (const m of c.methods) {
      elements.push(p.text(m.text, rhu(x + 8), rhu(my2), memFont, palette.text));
      my2 += lineH;
    }
  }
...
function endMarker(
  ...
  if (type === 'arrow') {
    ...
    return [
      p.path(
        `M ${rhu(a.x)} ${rhu(a.y)} L ${tip} L ${rhu(b.x)} ${rhu(b.y)}`,
        palette.textMuted,
        1.4,
      ),
    ];
  ...
  if (type === 'triangle') {
    ...
    return [
      p.path(
        `M ${tip} L ${rhu(a.x)} ${rhu(a.y)} L ${rhu(b.x)} ${rhu(b.y)} Z`,
        palette.textMuted,
        1.3,
  ...
  return [
    p.path(
      `M ${tip} L ${rhu(a.x)} ${rhu(a.y)} L ${rhu(near.x)} ${rhu(near.y)} L ${rhu(b.x)} ${rhu(b.y)} Z`,
      palette.textMuted,
      1.3,
      { fill },
    ),
