...
export function layoutEr(ir: ErDocument, theme: ResolvedTheme): LayoutResult {
  ...
  const sizeOf = (e: ErEntity): { w: number; h: number } => {
    const rowText = (a: ErEntity['attributes'][number]) =>
      `${a.type}  ${a.name}${a.key ? '  ' + a.key : ''}`;
    const w = Math.max(
      140,
      measureText(e.name, nameFont).width + 28,
      ...e.attributes.map((a) => measureText(rowText(a), rowFont).width + 28),
    );
  ...
  const nodes: GraphNode[] = ir.entities.map((e) => ({
    id: e.name,
    width: sizes.get(e.name)!.w,
    height: sizes.get(e.name)!.h,
  }));
  ...
  for (const r of ir.relations) {
    const a = laid.boxes.get(r.left),
      b = laid.boxes.get(r.right);
    if (!a || !b) continue;
    ...
    if (r.label) {
      ...
      elements.push(
        p.rect(
          { x: rhu(mx - w / 2), y: rhu(my - rowFont), width: rhu(w), height: rowFont + 4 },
          palette.background,
          palette.background,
          0,
        ),
      ...
      elements.push(
        p.text(r.label, rhuInt(mx), rhu(my - 1), rowFont, palette.textMuted, { anchor: 'middle' }),
      );
    }
  }
  ...
  for (const e of ir.entities) {
    ...
    elements.push(
      p.rect(
        { x: rhu(x), y: rhu(y), width: rhu(s.w), height: headH },
        palette.primary,
        palette.border,
        1.4,
        { rx: 4 },
    ...
    for (const a of e.attributes) {
      ...
      if (a.key)
        elements.push(
          p.text(a.key, rhu(x + s.w - 8), rhu(ry), rowFont, palette.secondary, {
            anchor: 'end',
            weight: 'bold',
          }),
        );
...
function crowFoot(
  ...
  const ux = (toward.x - at.x) / len,
    uy = (toward.y - at.y) / len; // unit toward other entity
  const px = -uy,
    py = ux; // perpendicular
