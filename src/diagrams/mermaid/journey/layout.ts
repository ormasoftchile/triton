...
export function layoutJourney(ir: JourneyDocument, theme: ResolvedTheme): LayoutResult {
  ...
  ir.sections.forEach((sec, si) =>
    sec.tasks.forEach((t) =>
      flat.push({ label: t.label, score: t.score, actors: t.actors, section: si }),
    ),
  );
  ...
  const scoreY = (s: number): number =>
    rhu(
      plotBottom -
        ((Math.max(SCORE_MIN, Math.min(SCORE_MAX, s)) - SCORE_MIN) / (SCORE_MAX - SCORE_MIN)) *
          plotH,
    );
  ...
  for (let s = SCORE_MIN; s <= SCORE_MAX; s++) {
    const y = scoreY(s);
    elements.push(
      p.path(`M ${plotLeft} ${y} L ${plotRight} ${y}`, palette.border, 1, { opacity: 0.4 }),
    );
    elements.push(
      p.text(
        String(s),
        plotLeft - 10,
        y + typography.smallFontSize * 0.35,
  ...
  ir.sections.forEach((sec, si) => {
    ...
    elements.push(
      p.text(
        sec.label,
        rhuInt((x0 + x1) / 2 - 3),
        margin + titleH + sectionBandH / 2 + typography.baseFontSize * 0.35,
        typography.baseFontSize,
        readableText(hue, theme),
  ...
  if (nodePts.length > 1) {
    elements.push(
      p.path(`M ${nodePts.map((pt) => `${pt.x} ${pt.y}`).join(' L ')}`, palette.textMuted, 2, {
        opacity: 0.6,
      }),
    );
  }
  ...
  flat.forEach((f, i) => {
    ...
    elements.push(
      p.text(
        String(f.score),
        x,
        y + typography.baseFontSize * 0.35,
        typography.baseFontSize,
    ...
    elements.push(
      p.text(
        f.label,
        x,
        plotBottom + typography.smallFontSize + 10,
        typography.smallFontSize,
        palette.text,
    ...
    f.actors.forEach((a, ai) => {
      elements.push(
        p.text(
          a,
          x,
          plotBottom + typography.smallFontSize + 10 + (ai + 1) * (typography.smallFontSize + 4),
          typography.smallFontSize,
          palette.textMuted,
          { anchor: 'middle' },
        ),
  ...
  const totalH = rhuInt(
    plotBottom +
      typography.smallFontSize +
      10 +
      (maxActors + 1) * (typography.smallFontSize + 4) +
      margin,
  );
