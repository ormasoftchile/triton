...
export function layoutC4(ir: C4Document, theme: ResolvedTheme): LayoutResult {
  ...
  const prep = new Map<
    string,
    { labelInfo: FormattedTextLines; descrInfo?: FormattedTextLines | undefined; h: number }
  >();
  for (const n of ir.nodes) {
    const labelInfo = measureFormattedText(n.label, typography.baseFontSize, font, NODE_W - 20, 4);
    const descrInfo = n.descr
      ? measureFormattedText(n.descr, font, font, NODE_W - 20, 4)
      : undefined;
    const labelH = labelInfo.lineCount * rhuInt(typography.baseFontSize * 1.25);
    const descrH = descrInfo ? descrInfo.lineCount * rhuInt(font * 1.3) + 4 : 0;
    const h = 14 + labelH + 4 + descrH + 14 + (isPerson(n.kind) ? 10 : 0);
    prep.set(n.id, { labelInfo, descrInfo, h });
  }

  const nodes: GraphNode[] = ir.nodes.map((n) => ({
    id: n.id,
    width: NODE_W,
    height: prep.get(n.id)!.h,
  }));
  ...
  for (const bnd of sorted) {
    ...
    const bx = rhu(minX),
      ...
      bh = rhu(maxY - minY);
    elements.push(
      p.path(`M ${bx} ${by} h ${bw} v ${bh} h ${-bw} Z`, palette.textMuted, 1.3, { dash: '7 5' }),
    );
    elements.push(
      p.text(bnd.label, rhu(minX + 10), rhu(minY + 16), font, palette.textMuted, {
        weight: 'bold',
      }),
    );
  }
  ...
  for (const r of ir.rels) {
    ...
    elements.push(
      p.path(path, palette.textMuted, 1.4, {
        ...(r.ext ? { dash: '6 4' } : {}),
        markerEnd: ARROW_ID,
      }),
    );
    ...
    if (lbl) {
      ...
      elements.push(
        p.rect(
          { x: rhu(mx - w / 2), y: rhu(my - font), width: rhu(w), height: font + 4 },
          palette.background,
          palette.background,
          0,
        ),
      ...
      elements.push(
        p.text(lbl, rhuInt(mx), rhu(my - 1), font, palette.textMuted, { anchor: 'middle' }),
      );
    }
  }
  ...
  for (const n of ir.nodes) {
    ...
    const x = box.x,
      ...
      w = NODE_W,
      h = box.height;
    const fill = isExt(n.kind)
      ? palette.surface
      : isPerson(n.kind)
        ? palette.secondary
        : palette.primary;
    const txt = isExt(n.kind) ? palette.text : '#FFFFFF';
    const mutedTxt = isExt(n.kind) ? palette.textMuted : 'rgba(255, 255, 255, 0.8)';
    const rx = isPerson(n.kind) ? 16 : 6;
    elements.push(
      p.rect(
        { x: rhu(x), y: rhu(y), width: w, height: rhu(h) },
        fill,
        isExt(n.kind) ? palette.textMuted : fill,
        isExt(n.kind) ? 1.4 : 0,
        { rx },
      ),
    );
    let ty = y + 16;
    const labelH = pr.labelInfo.lineCount * rhuInt(typography.baseFontSize * 1.25);
    elements.push(
      ...renderFormattedText(
        p,
        pr.labelInfo,
    ...
    elements.push(
      p.text(
        ...
        rhu(ty),
        font,
        isExt(n.kind) ? palette.textMuted : '#FFFFFF',
        { anchor: 'middle' },
      ),
    );
    ty += rhuInt(font * 1.3) + 2;
    if (pr.descrInfo) {
      const descrH = pr.descrInfo.lineCount * rhuInt(font * 1.3);
      elements.push(
        ...renderFormattedText(
          p,
          pr.descrInfo,
          ...
          font,
          isExt(n.kind) ? palette.textMuted : '#FFFFFF',
          isExt(n.kind) ? palette.textMuted : 'rgba(255, 255, 255, 0.8)',
  ...
  const allRects: Rect[] = ir.nodes.map((n) => {
    const b = laid.boxes.get(n.id)!;
    return { x: b.x, y: b.y + yOff, width: b.width, height: b.height };
  });
