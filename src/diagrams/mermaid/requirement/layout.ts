...
export function layoutRequirement(ir: RequirementDocument, theme: ResolvedTheme): LayoutResult {
  ...
  for (const r of ir.relations) {
    ...
    elements.push(
      p.rect(
        { x: rhu(mx - w / 2), y: rhu(my - fieldFont), width: rhu(w), height: fieldFont + 4 },
        palette.background,
        palette.background,
        0,
      ),
    ...
    elements.push(
      p.text(`«${r.type}»`, rhuInt(mx), rhu(my - 1), fieldFont, palette.secondary, {
        anchor: 'middle',
      }),
    );
  }
  ...
  for (const n of ir.nodes) {
    ...
    elements.push(
      p.rect(
        { x: rhu(x), y: rhu(y), width: BOX_W, height: rhu(box.height) },
        palette.surface,
        palette.primary,
        1.4,
        { rx: 6 },
    ...
    elements.push(
      p.path(
        `M ${rhu(x)} ${rhu(y + headH)} L ${rhu(x + BOX_W)} ${rhu(y + headH)}`,
        palette.border,
        1,
      ),
    );
    ...
    for (const ln of prep.lines) {
      elements.push(p.text(ln, rhu(x + 10), rhu(fy), fieldFont, palette.text));
      fy += lineH;
    }
  }
  ...
  const scene: Scene = applyOverlays(
    {
      viewBox: {
        x: 0,
        y: 0,
        width: rhuInt(laid.width + margin),
        height: rhuInt(laid.height + margin),
      },
