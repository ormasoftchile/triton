...
function parse(input: string): CQueueDoc {
  ...
  for (const line of lines(input)) {
    ...
    if (t[0] === 'cells' || t[0] === 'items') {
      cells = t.slice(1).map((v) => (EMPTY.has(v) ? null : v));
      continue;
    }
    if (t[0] === 'front') {
      const n = Number(t[1]);
      if (Number.isFinite(n)) front = n;
      continue;
    }
    if (t[0] === 'rear') {
      const n = Number(t[1]);
...
export function layoutCQueue(doc: CQueueDoc, theme: ResolvedTheme): LayoutResult {
  ...
  const cellInputs: StripCell[] = doc.cells.map((v, i) =>
    v !== null ? { label: v, index: String(i) } : { fill: palette.background, index: String(i) },
  );
...
export const cqueue: DiagramModule<
  CQueueDoc & { version: string; metadata: Record<string, unknown> }
> = {
