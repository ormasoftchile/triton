...
export function layoutStack(doc: StackDoc, theme: ResolvedTheme): LayoutResult {
  ...
  elements.push(
    p.text('push / pop', actionX, origin.y - 9, font, palette.primary, {
      anchor: 'middle',
      weight: 'bold',
    }),
  );
  elements.push(
    p.path(
      `M ${rhu(actionX)} ${rhu(origin.y - topH + 8)} L ${rhu(actionX)} ${rhu(origin.y)}`,
      palette.primary,
      1.5,
      { markerEnd: ARROW_ID },
    ),
  ...
  if (filled > 0) {
    if (horizontal) {
      ...
      elements.push(
        p.path(
          `M ${rhu(origin.x - sideGap + 8)} ${rhu(cyTop)} L ${rhu(origin.x)} ${rhu(cyTop)}`,
          palette.secondary,
          1.5,
          { markerEnd: ARROW_ID },
        ),
      ...
      elements.push(
        p.text('top', origin.x - sideGap + 6, cyTop - 6, font, palette.secondary, {
          anchor: 'start',
          weight: 'bold',
        }),
      );
...
export const stack: DiagramModule<
  StackDoc & { version: string; metadata: Record<string, unknown> }
> = {
