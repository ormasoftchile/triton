...
export function layoutPacket(ir: PacketDocument, theme: ResolvedTheme): LayoutResult {
  ...
  const fieldTokens: readonly Color[] = [
    palette.primary,
    palette.secondary,
    palette.success,
    palette.warning,
  ...
  for (let r = 0; r < numRows; r++) {
    for (let c = 0; c <= BITS_PER_ROW; c += 8) {
      ...
      if (c < BITS_PER_ROW)
        elements.push(
          p.text(String(bit), rhu(x + 2), rhu(rowTop(r) - 6), bitFont, palette.textMuted),
        );
    }
  }
  ...
  fields.forEach((f, i) => {
    ...
    for (let r = Math.floor(f.start / BITS_PER_ROW); r <= Math.floor(f.end / BITS_PER_ROW); r++) {
      ...
      const y = rowTop(r);
      elements.push(
        p.rect({ x: rhu(x), y: rhu(y), width: rhu(w), height: rowH }, fill, accent, 1.4, { rx: 3 }),
      );
      ...
      elements.push(
        p.text(String(f.start), rhu(x + 3), rhu(y + bitFont + 1), bitFont, palette.textMuted),
      );
    }
  });
...
function placeFieldLabel(
  ...
  if (measureText(text, baseFont).width <= w - padX) {
    elements.push(
      p.text(text, rhuInt(cx), rhu(y + cellH / 2 + baseFont * 0.35), baseFont, color, {
        weight: 'bold',
        anchor: 'middle',
      }),
    );
  ...
  elements.push(
    p.text(label, rhuInt(cx), rhu(y + cellH / 2 + f * 0.35), f, color, {
      weight: 'bold',
      anchor: 'middle',
    }),
  );
