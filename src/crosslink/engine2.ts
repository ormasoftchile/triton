...
export function routeAndRenderCrossLinks2(
  ...
  for (const link of sortedLinks) {
        ...
        for (const spBase of wallPorts(srcAnchor.bounds, ss)) {
          for (const dpBase of wallPorts(dstAnchor.bounds, ds)) {
            ...
            const score =
              ...
              portConflictPenalty(srcKey, ss, sp, dstKey, ds, dp, committed) +
    ...
    const strokeWidth = link.style === 'thick' ? (edgeTheme.strokeWidth + 0.5) * 2 : undefined; // undefined → use default at emission
 ...
 */
function sampleCubicBezierPts(p0: Point, p1: Point, p2: Point, p3: Point, n: number): Point[] {
  const pts: Point[] = [];
  for (let i = 0; i <= n; i++) {
    const t = i / n;
    const t2 = t * t,
